...
class CustomerTestClass(TestCase):
    # Testing class
    # We run all in one function so we can re-use database artefacts between tests
    # Django automatically makes a test database, so the actual database in settings.py is not affected
    ...
    def Test0_employee_creation(self):
        validated_data = {"username": "employee", "password": "employee",
    ...
    def Test1_customer_creation(self):
        ...
        customer_serializer = CustomerSerializer(
            data=validated_data, context=self.get_serializer_context())
    ...
    def Test2_organisation_department_creation(self):
        ...
        serializer = CustomerSerializer(
            c, context=self.get_serializer_context())
        self.assertEqual(
            serializer.data["department_name"], "Ministry of Silly Walks")
        self.assertEqual(
            serializer.data["organisation_name"], "Peaky Blinders")
    ...
    def Test3_invoice_creation(self):
        ...
        serializer = CustomerSerializer(
            self.get_customer(), context=self.get_serializer_context())
