...
class Department(models.Model):
    # Internal department that an employee or customer can be associated with
    # Only field is name, which is primarily for filtering/searching purpose
    # Client has no indicated at this time any preference for further department-based functionality
...
class Organisation(models.Model):
    # Organisation a customer represents/is affiliated with. Only name like department.
    name = models.CharField(max_length=255, null=False,
                            blank=False, unique=True)
...
class Customer(models.Model):
    ...
    # We want the image to have a random uuid, and reside under the customers folder
    def imagename(self, filename):
        extension = splitext(filename)[1]
        return "customers" + "/" + uuid4().hex + extension
    photo = models.ImageField(
        upload_to=imagename, default="default.png", null=True, blank=True)
    department = models.ForeignKey(
        Department, on_delete=models.SET_NULL, null=True)
    organisation = models.ForeignKey(
        Organisation, on_delete=models.SET_NULL, null=True)
    tag = models.CharField(max_length=255, null=True, blank=True)
    gender = models.CharField(max_length=1, validators=[
                              MinLengthValidator(1)], null=True, blank=True)

    # Helper functions that operate on the model
    ...
    # Send an email to any employee watching this customer, notifying them that this customer has been updated
    def update_watchers(self):
        customer = self
        customer_watchers = CustomerWatcher.objects.filter(
    ...
    # These simple calculation functions are needed here so CustomerSerializer can access them,
    # and the customer viewset can filter on them
    ...
    def get_total_overdue(self):
        invoices = Invoice.objects.filter(
            customer=self, date_paid=None, date_due__lt=datetime.date.today())
...
class Employee(models.Model):
    ...
    id = models.OneToOneField(
        settings.AUTH_USER_MODEL, primary_key=True, on_delete=CASCADE, db_column='id')
    job_title = models.CharField(max_length=255, null=True, blank=True)
    phone = models.CharField(max_length=255, null=True, blank=True)

    # Generate uuid name for employee photo
    def imagename(self, filename):
        extension = splitext(filename)[1]
...
class CustomerWatcher(models.Model):
    # Presence of this object means that the employee is watching the customer
...
class CustomerOwner(models.Model):
    # Presence of this object means that the employee owns this customer
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE)
    employee = models.ForeignKey(Employee, on_delete=models.CASCADE)
...
class Invoice(models.Model):
    ...
    pdf = models.FileField(upload_to=pdfname, validators=[
                           FileExtensionValidator(['pdf'])], null=True, blank=True)
